...
export default function ShoppingList() {
  ...
  const getButtonClass = (buttonType) => {
    const activeClass =
      "px-3 py-1 rounded border text-sm transition-colors bg-blue-600 text-white border-blue-600";
    const inactiveClass =
      "px-3 py-1 rounded border text-sm transition-colors bg-white text-gray-800 border-gray-300 hover:bg-gray-300";
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <p>Sort by:</p>
        <button className={getButtonClass("name")} onClick={handleSortByName}>
          Name
        </button>
        <button
